...
function human(n){return n==null?'-':String(n)}
...
export default function AdminMapsPage(){
  ...
  async function editLimit(limit){
    const newLimit = prompt('New limit value (integer)', String(limit.limitValue))
  ...
  async function downloadHistory(serviceType){
    try{
      ...
      a.href = url; a.download = `${serviceType}-maps-usage.csv`; document.body.appendChild(a); a.click(); a.remove();
        ...
        <section className="grid grid-cols-3 gap-4 mb-6">
          {usage && usage.length ? usage.map(u=> (
            <div key={u.serviceType} className="p-4 border rounded">
              ...
              <div className="text-xs text-gray-600">period: {u.period || 'month'}</div>
            </div>
              ...
              <tbody>
                {limits.map(l=> (
                  <tr key={l.serviceType} className="border-t">
                    ...
                    <td>{l.status}</td>
                    <td>
                      <button className="btn mr-2" onClick={()=>editLimit(l)}>Edit</button>
                      <button className="btn mr-2" onClick={()=>toggle(l.serviceType, l.status !== 'paused')}>{l.status==='paused' ? 'Resume':'Pause'}</button>
                      <button className="btn" onClick={()=>downloadHistory(l.serviceType)}>Download</button>
                    </td>
                  </tr>
                ))}
              </tbody>
