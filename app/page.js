...
function ValueCard({ icon, title, description, className = "" }) {
  return (
    <div
      className={`text-center p-6 bg-sand-100 dark:bg-ink/50 rounded-xl border border-sand-200 dark:border-slate-700/30 hover:shadow-md transition-shadow ${className}`}
    >
      ...
      <h3 className="text-base font-semibold text-ink dark:text-sand-50 mb-2 uppercase tracking-wide">
        {title}
      </h3>
...
export default function Home() {
  return (
    <div>
      <h1 className="sr-only">CASA MX — Tu ruta, tu decisión</h1>
      ...
      <section className="py-16 px-4 bg-white dark:bg-ink/80">
        <div className="container max-w-6xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <ValueCard
              icon={ICONS.transparencia}
              title="Transparencia"
              description="Información clara para todos."
            />
            <ValueCard
              icon={ICONS.negociacion}
              title="Negociación"
              description="Acuerdos justos entre personas."
            />
            <ValueCard
              icon={ICONS.datos}
              title="Datos reales"
              description="Decisiones basadas en información real."
            />
            <ValueCard
              icon={ICONS.confianza}
              title="Confianza"
              description="Seguridad y tranquilidad en cada paso."
            />
            <ValueCard
              icon={ICONS.equidad}
              title="Equidad"
              description="Un mercado justo para todos."
