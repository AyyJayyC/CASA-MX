...
const REASON_LABELS = Object.fromEntries(
  RETIRE_REASONS.map((r) => [r.value, r.label]),
);
const REASON_COLORS = Object.fromEntries(
  RETIRE_REASONS.map((r) => [r.value, r.color]),
);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-xl shadow-xl max-w-md w-full p-6 space-y-4">
        <h3 className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">
          ¿Por qué retiras esta propiedad?
        </h3>
        ...
        <div className="flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-2 text-xs font-medium border border-neutral-300 dark:border-neutral-600 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors"
          >
            Cancelar
          </button>
          <button
...
function DraftCard({ property, onRetire }) {
  let warnings = [];
  try {
    warnings = JSON.parse(property.inventoryNotes || "{}")?.warnings || [];
  } catch {}
  ...
  return (
    <div className="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-xl space-y-2">
      ...
      <div className="flex items-center gap-3 text-xs text-neutral-600 dark:text-neutral-400">
        ...
        <span className={photoCount === 0 ? "text-red-500 font-medium" : ""}>
          {photoCount} fotos
        </span>
      </div>
      ...
      {warnings.length > 0 && (
        <div className="text-xs text-amber-700 dark:text-amber-400 space-y-0.5">
          ...
          {warnings.length > 3 && (
            <p className="text-amber-500">+{warnings.length - 3} más</p>
          )}
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        <div className="flex gap-1">
          {LISTING_TYPE_FILTERS.map((f) => (
            <button
              key={f.key}
              onClick={() => setListingTypeFilter(f.key)}
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-36 bg-neutral-100 dark:bg-neutral-800 rounded-xl animate-pulse"
            />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 text-neutral-400">
          ...
          <Link
            ...
            className="text-sm text-clay hover:underline mt-2 inline-block"
          >
