...
export default function AgencyPage() {
  ...
  const handleCopyInvite = () => {
    ...
    navigator.clipboard
      .writeText(`${FRONTEND_URL}/register?agencia=${agency.referralCode}`)
      .then(() => {
        setCopied(true);
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Link href="/login" className="text-clay-400 font-medium">
          Iniciar sesión
        </Link>
      </div>
    );
      ...
      <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 flex items-center justify-center">
        <div className="text-center bg-white dark:bg-neutral-900 rounded-xl p-8 border border-neutral-200 dark:border-neutral-800 shadow-sm max-w-md">
          ...
          <button
            ...
            className="px-4 py-2 bg-clay text-white rounded-lg"
          >
            Reintentar
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950">
        <div className="max-w-4xl mx-auto px-4 py-10 space-y-6">
          <div>
            <h1 className="text-3xl font-bold">🏢 {membership.agency.name}</h1>
            <p className="text-neutral-500 mt-1">Eres agente de esta agencia</p>
          </div>
          <div className="p-6 bg-white dark:bg-neutral-900 border rounded-xl text-center space-y-3">
            <p className="text-5xl">🏢</p>
            <p className="text-lg font-medium">
              Perteneces a <strong>{membership.agency.name}</strong>
            </p>
            <p className="text-sm text-neutral-500">
              Las propiedades que publiques mostrarán el nombre de la agencia.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950">
        <div className="max-w-4xl mx-auto px-4 py-10 text-center space-y-6">
          ...
          <p className="text-neutral-500 max-w-md mx-auto">
            Trabajamos directamente con cada agencia para ofrecerte el mejor
            plan. Contáctanos por WhatsApp y te ayudamos a empezar.
          </p>
          {pricing && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto mt-4">
              {pricing.plans.map((p) => (
                <div
                  key={p.name}
                  className="p-5 bg-white dark:bg-neutral-900 border rounded-xl text-center space-y-2"
                >
                  <p className="font-bold text-lg">{p.label}</p>
                  <p className="text-2xl font-bold text-clay-400">
                    ${p.price.toLocaleString("es-MX")}{" "}
                    <span className="text-sm text-neutral-500">/mes</span>
                  </p>
                  <p className="text-sm text-neutral-500">
                    {p.agents} agentes · {p.leads} leads
                  </p>
                  <p className="text-xs text-neutral-400">
                    +${pricing.extraAgentCost} MXN por agente extra
                  </p>
                </div>
              ))}
            </div>
          ...
          <a
            href={`https://wa.me/${WHATSAPP}?text=Hola,%20me%20interesa%20registrar%20mi%20agencia%20en%20Casa-MX`}
            target="_blank"
  ...
  const pct =
    limit > 0 ? Math.min(Math.round((agentCount / limit) * 100), 100) : 0;
      ...
      <div className="max-w-4xl mx-auto px-4 py-10 space-y-6">
        ...
        <div className="p-6 bg-white dark:bg-neutral-900 border rounded-xl space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold ${agency.billingActive ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-neutral-100 text-neutral-600"}`}
              >
                {agency.billingActive
                  ? "Activo"
                  : new Date(agency.subscriptionEnds).getTime() < Date.now()
                    ? "Vencido"
                    : "Inactivo"}
              </span>
            ...
            <p className="text-sm text-neutral-500">
              ${planPrice.toLocaleString("es-MX")} MXN / mes
            </p>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="font-medium">
                Agentes: {agentCount} / {limit === 999 ? "∞" : limit}
              </span>
              {agency.billingActive && (
                <span className="text-neutral-500">{pct}% usado</span>
              )}
            </div>
            <div className="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-3 overflow-hidden">
              <div
                className="h-full rounded-full bg-clay-400 transition-all"
                style={{ width: `${pct}%` }}
              />
            </div>
          </div>
          ...
          {agency.subscriptionEnds && (
            <p
              className={`text-xs ${new Date(agency.subscriptionEnds).getTime() < Date.now() ? "text-red-500 font-medium" : "text-neutral-500"}`}
            >
              {new Date(agency.subscriptionEnds).getTime() < Date.now()
                ? "⚠️ Suscripción vencida el"
                : "Próximo cobro:"}{" "}
        ...
        <div className="p-6 bg-white dark:bg-neutral-900 border rounded-xl space-y-4">
          ...
          <p className="text-sm text-neutral-500">
            Los agentes que se registren con tu enlace se vincularán
            automáticamente a tu agencia.
          </p>
          {agency.billingActive && agency.referralCode ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <code className="flex-1 px-4 py-3 bg-neutral-100 dark:bg-neutral-800 rounded-lg text-lg font-mono font-bold text-clay-600 dark:text-clay-400 text-center tracking-wider">
                  {agency.referralCode}
                </code>
                <button
              ...
              <div className="flex gap-2">
                <a
                  href={`https://wa.me/?text=${encodeURIComponent("Únete a mi agencia en Casa-MX.com: " + FRONTEND_URL + "/register?agencia=" + agency.referralCode)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                <a
                  href={`mailto:?subject=Únete a mi agencia en Casa-MX&body=Regístrate aquí: ${FRONTEND_URL}/register?agencia=${agency.referralCode}`}
              ...
              {!showAddAgent ? (
                ...
                <div className="p-4 border border-clay-200 dark:border-clay-800 rounded-lg space-y-3 bg-clay-50/50 dark:bg-clay-900/10">
                  ...
                  {addAgentError && (
                    <p className="text-xs text-red-600">{addAgentError}</p>
                  )}
                  <div className="flex gap-2">
                    <button
            ...
            <p className="text-sm text-neutral-500">
              Tu plan está inactivo. Contacta al administrador para activarlo.
            </p>
          )}
        </div>
          ...
          {agentData.agents?.length > 0 ? (
            <div className="space-y-2">
              {agentData.agents.map((a) => (
                <div
                  key={a.id}
                  className="flex items-center gap-4 py-3 border-b border-neutral-100 dark:border-neutral-800 last:border-0"
                >
