...
import { getBuyers, createBuyer, updateBuyer, deleteBuyer } from '@/lib/api/crm';

// ─── Status column config for pipeline ─────────────────────────────
const PIPELINE_COLUMNS = [
  { status: 'disponible', label: 'Disponible', color: 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300', border: 'border-green-200 dark:border-green-800' },
  { status: 'preventa', label: 'Preventa', color: 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300', border: 'border-purple-200 dark:border-purple-800' },
  { status: 'bajo_promesa', label: 'Bajo promesa', color: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300', border: 'border-blue-200 dark:border-blue-800' },
  { status: 'vendido', label: 'Vendido', color: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300', border: 'border-red-200 dark:border-red-800' },
  { status: 'rentado', label: 'Rentado', color: 'bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300', border: 'border-teal-200 dark:border-teal-800' },
  { status: 'en_remodelacion', label: 'En remodelación', color: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300', border: 'border-amber-200 dark:border-amber-800' },
  { status: 'retirado', label: 'Retirado', color: 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400', border: 'border-gray-200 dark:border-gray-700' },
  { status: 'incompleto', label: 'Borrador', color: 'bg-amber-50 dark:bg-amber-900/10 text-amber-600 dark:text-amber-400', border: 'border-amber-100 dark:border-amber-800' },
];
...
function PipelineBoard({ properties, loading }) {
  if (loading) {
    return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-48 bg-neutral-100 dark:bg-neutral-800 rounded-xl animate-pulse" />)}
    </div>;
  }

  const byStatus = {};
  PIPELINE_COLUMNS.forEach(c => { byStatus[c.status] = []; });
    ...
    <div className="overflow-x-auto pb-4">
      <div className="flex gap-4 min-w-[1100px]">
        {PIPELINE_COLUMNS.map(col => (
          <div key={col.status} className={`flex-1 min-w-[160px] p-3 rounded-xl border ${col.border} bg-white dark:bg-neutral-900/50`}>
...
// ─── Calendar Widget ─────────────────────────────────────────────────
function CalendarWidget({ properties, loading, year, month, onPrevMonth, onNextMonth }) {
  if (loading) return <div className="h-64 bg-neutral-100 dark:bg-neutral-800 rounded-xl animate-pulse" />;
  ...
  return (
    <div className="p-4 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-xl">
      <div className="flex items-center justify-between mb-3">
        <button onClick={onPrevMonth} className="px-2 py-1 text-xs rounded hover:bg-neutral-100 dark:hover:bg-neutral-800">◀</button>
        <h3 className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{MONTHS[month]} {year}</h3>
        <button onClick={onNextMonth} className="px-2 py-1 text-xs rounded hover:bg-neutral-100 dark:hover:bg-neutral-800">▶</button>
      </div>
      <div className="grid grid-cols-7 gap-0.5">
        {dayNames.map(d => <div key={d} className="text-center text-[10px] font-medium text-neutral-400 py-1">{d}</div>)}
        {cells.map((d, i) => (
          <div key={i} className={`aspect-square rounded flex flex-col items-center justify-start pt-0.5 text-xs ${
            ...
            {d && propsByDay[d]?.length > 0 && (
              <span className="text-[9px] text-clay mt-0.5">{propsByDay[d].length}</span>
            )}
          </div>
        ))}
      </div>
      <p className="text-[10px] text-neutral-400 mt-2 text-center">
        Se muestran fechas de creación, disponibilidad y actualización de propiedades.
      </p>
    </div>
  );
}
...
export default function CRMPage() {
  ...
  if (!isAuthenticated) {
    return <div className="p-10 text-center text-neutral-500">Inicia sesión para acceder al CRM.</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 p-4 sm:p-6 space-y-5">
      ...
      {tab === 'pipeline' && <PipelineBoard properties={properties} loading={loading} />}
      ...
      {tab === 'calendar' && (
        <CalendarWidget
          ...
          year={calYear}
          month={calMonth}
          onPrevMonth={() => calMonth === 0 ? (setCalMonth(11), setCalYear(y => y - 1)) : setCalMonth(m => m - 1)}
          onNextMonth={() => calMonth === 11 ? (setCalMonth(0), setCalYear(y => y + 1)) : setCalMonth(m => m + 1)}
        />
      )}
...
function BuyersPlaceholder() {
  return (
    <div className="text-center py-16 text-neutral-400">
      ...
      <p className="font-medium text-neutral-600 dark:text-neutral-400">Lista de compradores próximamente</p>
      <p className="text-sm mt-1">Podrás agregar y gestionar compradores interesados.</p>
    </div>
  );
}
...
function BuyersList({ isAuthenticated }) {
  ...
  useEffect(() => { if (isAuthenticated) loadBuyers(); }, [isAuthenticated]);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        preferredZones: form.preferredZones ? form.preferredZones.split(',').map(s => s.trim()).filter(Boolean) : [],
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setShowForm(!showForm); setEditing(null); setForm({ name: '', phone: '', email: '', budgetMin: '', budgetMax: '', preferredZones: '', propertyType: '', notes: '' }); }}
      ...
      {showForm && (
        <div className="p-4 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-xl space-y-3">
          <h3 className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{editing ? 'Editar comprador' : 'Nuevo comprador'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <input placeholder="Email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
              className="px-3 py-2 text-sm border border-neutral-300 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100" />
            <input placeholder="Presupuesto mín" type="number" value={form.budgetMin} onChange={e => setForm(f => ({ ...f, budgetMin: e.target.value }))}
              className="px-3 py-2 text-sm border border-neutral-300 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100" />
            <input placeholder="Presupuesto máx" type="number" value={form.budgetMax} onChange={e => setForm(f => ({ ...f, budgetMax: e.target.value }))}
              className="px-3 py-2 text-sm border border-neutral-300 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100" />
            <input placeholder="Tipo de propiedad" value={form.propertyType} onChange={e => setForm(f => ({ ...f, propertyType: e.target.value }))}
              className="px-3 py-2 text-sm border border-neutral-300 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100" />
          </div>
          <input placeholder="Zonas preferidas (separadas por coma)" value={form.preferredZones} onChange={e => setForm(f => ({ ...f, preferredZones: e.target.value }))}
          ...
          <textarea placeholder="Notas" rows={2} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
            className="w-full px-3 py-2 text-sm border border-neutral-300 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100" />
      ...
      {loading ? (
        <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-12 bg-neutral-100 dark:bg-neutral-800 rounded-lg animate-pulse" />)}</div>
      ) : buyers.length === 0 ? (
        <div className="text-center py-12 text-neutral-400"><p className="text-3xl mb-2">👥</p><p>No hay compradores aún.</p></div>
      ) : (
            ...
            <tbody>
              {buyers.map(b => (
                <tr key={b.id} className="border-b border-neutral-100 dark:border-neutral-800">
                  <td className="py-2 px-2 font-medium text-neutral-800 dark:text-neutral-200">{b.name}</td>
                  <td className="py-2 px-2 text-neutral-600 dark:text-neutral-400">
                    {b.phone && <span className="block">{b.phone}</span>}
                    {b.email && <span className="block text-[10px]">{b.email}</span>}
                  </td>
                  <td className="py-2 px-2 text-neutral-600 dark:text-neutral-400 whitespace-nowrap">{formatBudget(b)}</td>
                  <td className="py-2 px-2 text-neutral-600 dark:text-neutral-400 max-w-[120px] truncate">{(b.preferredZones || []).join(', ') || '—'}</td>
                  <td className="py-2 px-2 text-neutral-600 dark:text-neutral-400">{b.propertyType || '—'}</td>
                  <td className="py-2 px-2 text-neutral-600 dark:text-neutral-400 max-w-[100px] truncate">{b.notes || '—'}</td>
                  <td className="py-2 px-2">
                    <button onClick={() => handleEdit(b)} className="text-xs text-clay hover:underline mr-2">Editar</button>
                    <button onClick={() => handleDelete(b.id)} className="text-xs text-red-500 hover:underline">Eliminar</button>
                  </td>
                </tr>
              ))}
            </tbody>
