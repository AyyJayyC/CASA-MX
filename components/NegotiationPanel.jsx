...
export default function NegotiationPanel({ applicationId, originalRent, applicantId, landlordId }) {
  ...
  if (loading) return <p className="text-sm text-neutral-500">Cargando negociación...</p>;

  const latestOffer = negotiation?.offers?.slice(-1)[0];
  const isOpen = negotiation?.status === 'open';
  const canRespond = isOpen && latestOffer?.status === 'pending' && latestOffer?.authorId !== user?.id;
  const canCounter = isOpen && latestOffer?.status === 'pending' && latestOffer?.authorId !== user?.id;
  ...
  return (
    <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 p-5 space-y-4 bg-white dark:bg-neutral-900">
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-neutral-800 dark:text-white">Negociación de renta</h3>
        {negotiation && (
      ...
      <p className="text-sm text-neutral-600 dark:text-neutral-400">
        ...
        {negotiation?.status === 'accepted' && negotiation.finalRent && (
          <> → Acordada: <strong className="text-green-600 dark:text-green-400">{formatMXN(negotiation.finalRent)}/mes</strong></>
        )}
      </p>
          ...
          {negotiation.offers.map((offer) => (
            <div key={offer.id} className={`rounded-lg p-3 text-sm ${
              ...
              <p className="text-xs text-neutral-400 mt-1">
                ...
                <span className={offer.status === 'accepted' ? 'text-green-600' : offer.status === 'rejected' ? 'text-red-600' : 'text-neutral-400'}>
                  {offer.status === 'pending' ? 'Pendiente' : offer.status === 'accepted' ? 'Aceptada' : offer.status === 'rejected' ? 'Rechazada' : 'Contraofertada'}
                </span>
              </p>
            </div>
          ))}
...
function OfferForm({ label, proposedRent, message, onChangeRent, onChangeMessage, onSubmit, onCancel, loading }) {
  return (
