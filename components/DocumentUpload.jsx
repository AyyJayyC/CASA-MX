...
function validateFile(file) {
  ...
  if (!ALLOWED_TYPES.has(file.type)) {
    return {
      valid: false,
      error: `File type "${file.type}" not accepted. Use PDF, JPEG, PNG, or WebP.`,
    };
  }
  ...
  const handleUpload = async (fieldName) => {
    ...
    try {
      ...
      const res = await fetch(
        `${BACKEND_URL}/documents/upload/${applicationId}`,
        {
          method: "POST",
          credentials: "include",
          headers,
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-neutral-500 dark:text-neutral-400">
        Todos los documentos son privados y sólo serán visibles para el
        propietario.
      </p>
