...
const CarouselSlide = React.memo(function CarouselSlide({ property, isActive }) {
  ...
  const location = [property.colonia, property.ciudad, property.estado]
    .filter(Boolean)
    .join(", ");

  return (
    <div
      className={`absolute inset-0 transition-opacity duration-700 first:opacity-100 first:z-10 ${isActive ? "opacity-100 z-10" : "opacity-0 z-0"}`}
    >
      ...
      <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-ink/50 to-transparent px-3 py-2 md:px-5 md:py-2.5 z-10">
        ...
        <div className="hidden md:flex items-center gap-3 text-white">
          <span className="shrink-0 px-2 py-0.5 rounded-full text-[11px] font-bold bg-clay">
            🔥 Promocionado
          </span>
          <span className="font-semibold text-sm truncate max-w-[280px]">
            {property.title}
          </span>
          <span className="text-white/60 text-xs truncate max-w-[200px] hidden lg:block">
            {location}
          </span>
          <Link
            href={`/properties/${property.id}`}
            className="ml-auto shrink-0 px-3 py-1 bg-white/15 hover:bg-white/25 rounded-lg text-xs font-semibold transition-colors"
          >
            {price} Ver ›
          </Link>
        </div>
        ...
        <div className="md:hidden flex items-center justify-between text-white">
          <span className="shrink-0 px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-clay">
            🔥
          </span>
          <span className="text-xs font-semibold truncate mx-2 flex-1">
            {property.title}
          </span>
          <Link
            href={`/properties/${property.id}`}
            className="shrink-0 px-2 py-0.5 bg-white/15 hover:bg-white/25 rounded text-[11px] font-semibold"
          >
            {price} ›
          </Link>
        </div>
    ...
    <>
          ...
          <div className="container max-w-3xl text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-2 justify-center">
              <Link
                ...
                className="px-4 py-1.5 md:px-5 md:py-2 bg-clay hover:bg-clay-500 text-white font-semibold text-xs md:text-sm rounded-lg transition-all shadow-md"
              >
                Publicar propiedad
              </Link>
              <Link
                href="/properties"
                className="px-4 py-1.5 md:px-5 md:py-2 bg-white/15 hover:bg-white/25 border border-white/30 text-white font-semibold text-xs md:text-sm rounded-lg transition-all"
              >
      ...
      <div className="block md:hidden bg-gradient-to-br from-sand-50 to-sand-100 dark:from-ink dark:to-ink/90 px-4 py-5 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-2 justify-center">
          <Link
            href="/publish-property"
            className="px-4 py-2 bg-clay hover:bg-clay-500 text-white font-semibold text-sm rounded-lg transition-all"
          >
            Publicar propiedad
          </Link>
          <Link
            href="/properties"
            className="px-4 py-2 bg-white dark:bg-ink border-2 border-sand-200 dark:border-slate-700 text-ink dark:text-sand-50 hover:border-clay font-semibold text-sm rounded-lg transition-all"
          >
            Explorar propiedades
          </Link>
        </div>
