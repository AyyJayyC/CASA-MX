  ...
  return (
    <div>
      <button onClick={() => setOpen(true)} className="
        ...
        transition-colors
      ">Solicitar dirección</button>

      {open && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
          <div className="bg-white dark:bg-neutral-900 p-6 rounded-xl max-w-md w-full mx-4 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100">Solicitar dirección</h3>
              <button
            ...
            {!success ? (
              <ContactRequestForm propertyId={propertyId} onSuccess={(s) => setSuccess(s)} />
            ) : (
              <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg text-green-800 dark:text-green-300 text-sm">
                ...
                <p className="text-xs mt-1">El vendedor revisará tu solicitud y compartirá la dirección contigo.</p>
              </div>
            )}
          </div>
        </div>
