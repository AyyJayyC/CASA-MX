...
const OfferTrendChart = dynamic(() => import("./OfferTrendChart"), {
...
export default function CityMarketTable({
  cities,
  loading,
  onDrilldown,
      ...
      <div className="p-5 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl animate-pulse">
        ...
        <div className="space-y-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-8 bg-neutral-100 dark:bg-neutral-800 rounded"
            />
          ))}
        </div>
      </div>
  ...
  const toggleExpand = async () => {
    ...
    if (!colonias) {
      ...
      try {
        ...
        setColoniasError("Error al cargar colonias");
        setColonias([]);
      } finally {
      ...
      try {
        ...
        setTrendsError("Error al cargar tendencias");
        setTrends(null);
      } finally {
      ...
      try {
        ...
        setCompsError("Error al cargar comparables");
        setComps([]);
      } finally {
  ...
  return (
    <>
      ...
      >
        ...
        <td className="py-2 px-1.5 text-neutral-700 dark:text-neutral-300">
          {formatNumber(city.activeListings)}
        </td>
        <td className="py-2 px-1.5 text-neutral-700 dark:text-neutral-300">
          {formatCurrency(city.medianOfferPerSqm)}
        </td>
        <td className="py-2 px-1.5">
          <span
            className={`font-medium ${(city.momChange || 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
          >
            {city.momChange != null
              ? `${city.momChange > 0 ? "+" : ""}${city.momChange.toFixed(1)}%`
              : "—"}
          </span>
        </td>
        <td className="py-2 px-1.5 text-neutral-700 dark:text-neutral-300">
          {formatNumber(city.medianDaysToOffer)}
        </td>
        <td className="py-2 px-1.5 text-neutral-700 dark:text-neutral-300">
          {city.avgOffersPerProperty?.toFixed(1) || "—"}
        </td>
        <td className="py-2 px-1.5 text-neutral-700 dark:text-neutral-300">
          {formatPercentage(city.acceptanceRate)}
        </td>
        <td className="py-2 px-1.5">
          {city.staleCount > 0 ? (
        ...
        <td className="py-2 px-1.5">
          <span
            className={`px-1.5 py-0.5 rounded-full text-xs font-medium ${
              (city.activityScore || 0) >= 7
                ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300"
                : (city.activityScore || 0) >= 4
  ...
  if (!colonias || colonias.length === 0) {
    return (
      <p className="text-xs text-neutral-400 py-4">
        Sin datos por colonia para {ciudad}.
      </p>
    );
  }

  const sorted = [...colonias].sort(
    (a, b) => (b.medianOfferPerSqm || 0) - (a.medianOfferPerSqm || 0),
  );
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {sorted.map((col) => (
              ...
              >
                <td className="py-1.5 px-2 font-medium text-neutral-800 dark:text-neutral-200">
                  {col.colonia}
                </td>
                <td className="py-1.5 px-2 text-right text-neutral-600 dark:text-neutral-400">
                  {formatNumber(col.activeListings)}
                </td>
                <td className="py-1.5 px-2 text-right text-neutral-700 dark:text-neutral-300">
                  {formatCurrency(col.medianOfferPerSqm)}
                </td>
                <td className="py-1.5 px-2 text-right text-neutral-600 dark:text-neutral-400">
                  {formatNumber(col.medianDaysToOffer)}
                </td>
                <td className="py-1.5 px-2 text-right text-neutral-600 dark:text-neutral-400">
                  {col.totalOffers != null
                    ? formatNumber(col.totalOffers)
                    : "—"}
                </td>
                <td className="py-1.5 px-2 text-right text-neutral-700 dark:text-neutral-300">
                  {formatPercentage(col.acceptanceRate)}
                </td>
                ...
                <td className="py-1.5 px-2 text-right">
                  <span
                    className={`font-medium ${(col.momChange || 0) >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                  >
                    {col.momChange != null
                      ? `${col.momChange > 0 ? "+" : ""}${col.momChange.toFixed(1)}%`
                      : "—"}
                  </span>
                </td>
      ...
      <div className="p-4 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg animate-pulse">
        ...
        <div className="space-y-1">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="h-5 bg-neutral-100 dark:bg-neutral-700 rounded"
            />
          ))}
        </div>
      </div>
    ...
    <div className="p-4 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg">
      ...
      <div className="space-y-2">
        {comps.slice(0, 5).map((comp, i) => (
          <div
            key={i}
            className="flex items-center justify-between text-xs border-b border-neutral-100 dark:border-neutral-700 pb-1.5 last:border-0 last:pb-0"
          >
            ...
            <div className="text-right shrink-0 ml-3">
              <p className="font-semibold text-neutral-800 dark:text-neutral-200">
                {formatCurrency(comp.offerAmount)}
              </p>
              <p
                className={`text-xs ${comp.status === "accepted" ? "text-green-600 dark:text-green-400" : comp.status === "rejected" ? "text-red-600 dark:text-red-400" : "text-amber-600 dark:text-amber-400"}`}
              >
                {comp.status === "accepted"
                  ? "Aceptada"
                  : comp.status === "rejected"
                    ? "Rechazada"
                    : comp.status === "countered"
                      ? "Contraoferta"
                      : comp.status}
