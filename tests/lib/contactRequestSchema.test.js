...
describe('contactRequestSchema', () => {
  it('accepts valid request with name and phone', () => {
    const r = contactRequestSchema.safeParse({ name: 'Juan Perez', phone: '5512345678' });
    expect(r.success).toBe(true);
  });

  it('accepts phone with +52 prefix', () => {
    const r = contactRequestSchema.safeParse({ name: 'Maria', phone: '+525512345678' });
    expect(r.success).toBe(true);
  });

  it('accepts phone with 52 prefix (no +)', () => {
    const r = contactRequestSchema.safeParse({ name: 'Carlos', phone: '525512345678' });
    expect(r.success).toBe(true);
  });

  it('accepts phone with formatting characters (transform before regex)', () => {
    const r = contactRequestSchema.safeParse({ name: 'Ana', phone: '55-1234-5678' });
    expect(r.success).toBe(true);
  });

  it('strips formatting via transform', () => {
    const r = contactRequestSchema.safeParse({ name: 'Pedro', phone: '+52 (55) 1234-5678' });
    expect(r.success).toBe(true);
    if (r.success) expect(r.data.phone).toBe('525512345678');
  });

  it('includes optional message', () => {
    const r = contactRequestSchema.safeParse({ name: 'Luis', phone: '5512345678', message: 'Me interesa esta propiedad' });
    expect(r.success).toBe(true);
    if (r.success) expect(r.data.message).toBe('Me interesa esta propiedad');
  });

  it('rejects name shorter than 2 chars', () => {
    const r = contactRequestSchema.safeParse({ name: 'A', phone: '5512345678' });
  ...
  it('rejects message over 500 chars', () => {
    const r = contactRequestSchema.safeParse({ name: 'Test', phone: '5512345678', message: 'x'.repeat(501) });
    expect(r.success).toBe(false);
  });
