import { propertySchema, financeOptionsSchema } from '../../lib/validation/propertySchema';
...
describe('propertySchema', () => {
  describe('for_sale', () => {
    ...
    it('rejects title too short (<5 chars)', () => {
      const r = propertySchema.safeParse({ ...validSaleProperty, title: 'Casa' });
      expect(r.success).toBe(false);
    });

    it('rejects missing description', () => {
      const r = propertySchema.safeParse({ ...validSaleProperty, description: '' });
      expect(r.success).toBe(false);
    });
    ...
    it('rejects invalid propertyType', () => {
      const r = propertySchema.safeParse({ ...validSaleProperty, propertyType: 'Castillo' });
      expect(r.success).toBe(false);
    });

    it('rejects negative bedrooms', () => {
      const r = propertySchema.safeParse({ ...validSaleProperty, bedrooms: -1 });
      expect(r.success).toBe(false);
    });

    it('rejects zero square meters', () => {
      const r = propertySchema.safeParse({ ...validSaleProperty, squareMeters: 0 });
      expect(r.success).toBe(false);
    });
  ...
  describe('for_rent', () => {
    ...
    it('rejects sale property with price=0 as rental (discriminated union)', () => {
      const r = propertySchema.safeParse({ ...validSaleProperty, listingType: 'for_rent' });
      expect(r.success).toBe(false);
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('rejects invalid listingType', () => {
      const r = propertySchema.safeParse({ ...validSaleProperty, listingType: 'invalid' });
      expect(r.success).toBe(false);
    });
