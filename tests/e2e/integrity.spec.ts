...
test.describe('E2E Integrity - Adversarial Tests', () => {
  ...
  test.describe('B - Input Validation: Invalid Data Must Be Rejected', () => {
    ...
    test('ADVERSARIAL: Invalid email format is rejected', async ({ page }) => {
      ...
      if (await emailInputs.count() > 0) {
        ...
        const hasError = await page.locator('text=email|invalid|required', { exact: false })
          .isVisible()
          .catch(() => false);
  ...
  test.describe('C - State Integrity: Application Cannot Be Submitted to Rented Property', () => {
    test('ADVERSARIAL: Cannot submit application when property status changes during process', async ({ page, request }) => {
      ...
      if (linkCount > 0) {
        ...
        if (propertyId) {
          ...
          const applicationForm = page.locator('button:has-text("Apply")')
    ...
    test('ADVERSARIAL: Property status changes are reflected in real-time', async ({ page }) => {
    ...
    test('ADVERSARIAL: Duplicate application attempt shows error message', async ({ page }) => {
  ...
  test.describe('D - Admin Operations: Only Admins Can Approve Applications', () => {
    test('ADVERSARIAL: Landlord cannot access other landlord\'s applications', async ({ page }) => {
      ...
      const currentUrl = page.url();
      const isAuthProtected = currentUrl.includes('/login') || 
                              currentUrl.includes('/dashboard');
      
      expect(isAuthProtected).toBeTruthy();
    });

    test('ADVERSARIAL: Application approval from unauthorized user fails at backend', async ({ request }) => {
      // Create a fake/invalid approval request
      const response = await request.post(`${BACKEND_URL}/applications/invalid-id`, {
        data: {
          status: 'approved',
        },
      });
      ...
      const isRejected = response.status() === 401 || response.status() === 404 || response.status() === 403;
  ...
  test.describe('E - Data Persistence: Changes Are Saved in Database', () => {
    test('ADVERSARIAL: Page refresh maintains application data', async ({ page }) => {
      // Navigate to properties
      await page.goto(`${FRONTEND_URL}/properties`);
    ...
    test('ADVERSARIAL: Logout clears user session properly', async ({ page, context }) => {
      // Navigate to login
      await page.goto(`${FRONTEND_URL}/login`);
