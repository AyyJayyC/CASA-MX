  ...
  try {
    ...
    if (currentUrl.includes("/login")) {
      ...
      const detail = accessToken
        ? `token present (domain: ${accessToken.domain}, path: ${accessToken.path}, httpOnly: ${accessToken.httpOnly})`
        : "token missing";
      throw new Error(
        `Auth failed — redirected to /login from ${url}. ${detail}. ` +
        `This usually means the backend rate-limited /auth/me (HTTP 429) ` +
        `or the token is stale from parallel test execution.`
      );
    }
