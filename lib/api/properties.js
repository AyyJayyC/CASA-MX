...
export async function getMyProperties(filters = {}) {
  try {
    ...
    const response = await fetchWithAuthRetry(`${BACKEND_URL}/properties/mine${queryParams.toString() ? `?${queryParams.toString()}` : ''}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    });
    ...
    if (!response.ok) {
      ...
      throw new Error(error?.error || 'Failed to fetch owned properties');
    }
...
export async function updateProperty(id, updates) {
  try {
    const response = await fetchWithAuthRetry(`${BACKEND_URL}/properties/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
export async function publishProperty(id) {
  const response = await fetchWithAuthRetry(`${BACKEND_URL}/properties/${id}/publish`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  });
...
export async function promoteProperty(id, tier, days) {
  const response = await fetchWithAuthRetry(`${BACKEND_URL}/properties/${id}/promote`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getLocationsCatalog() {
  const { getUnifiedCatalog } = await import('./locations.js');
 ...
 */
export async function bulkImportProperties(rows, defaultVisibility = 'private') {
  ...
  for (const row of rows) {
    try {
      ...
      const response = await fetchWithAuthRetry(`${BACKEND_URL}/properties?soft=true`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      if (!response.ok) {
        ...
        if (response.status === 429) {
          detail = 'Demasiadas solicitudes (límite: 100/15 min por cuenta). Espera e intenta de nuevo.';
        } else {
          detail = error?.details?.[0]?.message || error?.error || `HTTP ${response.status}`;
        }
        results.errors.push({
        ...
        const data = await response.json();
        if (data?.isIncomplete || data?.warnings?.length > 0) {
          results.incomplete++;
          results.errors.push({
            title: row.title || 'Sin título',
