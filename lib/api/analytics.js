...
export async function getMarketByCity(estado, listingType = 'for_sale') {
  ...
  return authFetch(`${BACKEND_URL}/admin/analytics/market-by-city?${params.toString()}`);
}
 ...
 *            momChange, compCount }]
 */
export async function getMarketByColonia(estado, ciudad, listingType = 'for_sale') {
  const params = new URLSearchParams();
  if (estado) params.set('estado', estado);
  if (ciudad) params.set('ciudad', ciudad);
  if (listingType) params.set('listingType', listingType);
  return authFetch(`${BACKEND_URL}/admin/analytics/market-by-colonia?${params.toString()}`);
}
 ...
 */
export async function getOfferTrends(estado, ciudad, colonia, listingType = 'for_sale', months = 12) {
  const params = new URLSearchParams();
  if (estado) params.set('estado', estado);
  if (ciudad) params.set('ciudad', ciudad);
  ...
  return authFetch(`${BACKEND_URL}/admin/analytics/offer-trends?${params.toString()}`);
}

/**
 * Offer behavior analysis: rates, ratios, distribution.
 * Returns: { avgOffersPerProperty, acceptanceRate, counterRate, offerToAskRatio }
 */
export async function getOfferAnalysis(estado, ciudad) {
  const params = new URLSearchParams();
  if (estado) params.set('estado', estado);
  if (ciudad) params.set('ciudad', ciudad);
  return authFetch(`${BACKEND_URL}/admin/analytics/offer-analysis?${params.toString()}`);
}
 ...
 */
export async function getComps(estado, ciudad, colonia, listingType = 'for_sale', limit = 20) {
  const params = new URLSearchParams();
  if (estado) params.set('estado', estado);
  if (ciudad) params.set('ciudad', ciudad);
  if (colonia) params.set('colonia', colonia);
