...
export async function getCarouselSlides() {
  const res = await fetch(`${BACKEND_URL}/carousel`, { credentials: 'include' });
...
export async function getMostViewedProperties(limit = 6) {
  const res = await fetch(`${BACKEND_URL}/properties/most-viewed?limit=${limit}`, { credentials: 'include' });
