...
async function authFetch(url, options = {}) {
  return fetch(url, {
    ...options,
    headers: csrfHeaders(options.headers || { 'Content-Type': 'application/json' }),
    credentials: 'include',
  });
}
...
export async function register(payload) {
  ...
  const response = await authFetch(`${BACKEND_URL}/auth/register`, {
    method: 'POST',
...
export async function login(payload) {
  const response = await authFetch(`${BACKEND_URL}/auth/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
...
export async function refreshAccessToken() {
  const response = await authFetch(`${BACKEND_URL}/auth/refresh`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
    body: JSON.stringify({}),
