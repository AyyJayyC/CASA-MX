...
const basePropertySchema = z.object({
  ...
  description: z.string().min(10, 'La descripción debe tener al menos 10 caracteres').max(10000, 'La descripción es demasiado larga'),
  address: z.string().min(5, 'La dirección es requerida').max(500, 'La dirección es demasiado larga'),
  estado: z.string().min(2, 'El estado es requerido').max(100, 'Estado inválido'),
  ciudad: z.string().min(2, 'La ciudad es requerida').max(100, 'Ciudad inválida'),
  colonia: z.string().min(2, 'La colonia es requerida').max(100, 'Colonia inválida'),
  ...
  bedrooms: numberInput(z.number().int().min(0, 'Las recámaras no pueden ser negativas').max(50, 'Máximo 50 recámaras')),
  bathrooms: numberInput(z.number().int().min(0, 'Los baños no pueden ser negativos').max(50, 'Máximo 50 baños')),
  squareMeters: numberInput(z.number().int().min(1, 'Los metros cuadrados deben ser > 0').max(1000000, 'Metros cuadrados exceden el límite')),
  parkingSpaces: optionalNumberInput(z.number().int().min(0, 'Los cajones de estacionamiento no pueden ser negativos').max(100, 'Máximo 100 cajones')),
  parkingType: z.enum(PARKING_TYPE_OPTIONS).optional(),
  halfBaths: optionalNumberInput(z.number().int().min(0).max(20, 'Máximo 20 medios baños')),
  miniSplits: optionalNumberInput(z.number().int().min(0, 'Los mini splits no pueden ser negativos').max(100, 'Máximo 100 mini splits')),
  condition: z.enum(PROPERTY_CONDITION_OPTIONS).optional(),
  yearBuilt: optionalNumberInput(z.number().int().min(1800, 'Año inválido').max(2027)),
  floors: optionalNumberInput(z.number().int().min(1, 'Los pisos deben ser > 0').max(100, 'Máximo 100 pisos')),
  lotSize: optionalNumberInput(z.number().int().min(0).max(10000000, 'Terreno excede el límite')),
  maintenanceFee: optionalNumberInput(z.number().min(0, 'La cuota no puede ser negativa').max(1000000)),
...
const salePropertySchema = basePropertySchema.extend({
  ...
  price: numberInput(z.number().min(1, 'El precio debe ser mayor a 0').max(999999999, 'Precio excede el límite máximo')),
...
export const propertyFormDefaults = {
  ...
  status: 'disponible',
  visibility: 'public',
  financeOptions: { cash: false, bankLoan: false, INFONAVIT: false, FOVISSSTE: false, paymentPlan: false, other: false },
  latitude: null,
  longitude: null,
  parkingType: null,
  halfBaths: null,
