...
export const PROPERTY_AMENITY_CATEGORIES = [
  ...
  },
  {
    ...
    items: [
      ...
      { value: 'Mini split', label: 'Mini split', emoji: '🌬️' },
  ...
  },
  {
    ...
    items: [
      ...
      { value: 'Seguridad 24/7', label: 'Seguridad 24/7', emoji: '🚨' },
...
export const PROPERTY_AMENITY_METADATA = PROPERTY_AMENITY_CATEGORIES.flatMap((category) =>
  category.items.map((item) => ({ ...item, category: category.id, categoryLabel: category.label }))
);
...
export function getServiceMeta(service) {
  return SERVICE_LOOKUP[service] || { value: service, label: service, emoji: '•' };
}

export function getAmenityMeta(amenity) {
  return AMENITY_LOOKUP[amenity] || { value: amenity, label: amenity, emoji: '•', category: 'other', categoryLabel: 'Otros' };
}
