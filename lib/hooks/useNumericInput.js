...
export default function useNumericInput({
  ...
  const [displayValue, setDisplayValue] = useState(toDisplayValue(value, formatDisplay));
  ...
  const handlers = formatDisplay
    ? {
        ...
        onChange: (event) => {
          ...
          const formatted = num === '' ? '' : Number(num).toLocaleString('es-MX');
        ...
        onBlur: () => {
          ...
          const formatted = num === 0 ? '0' : Number(num).toLocaleString('es-MX');
          setDisplayValue(formatted);
          onValueChange?.(num);
        },
      }
