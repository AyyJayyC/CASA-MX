...
export default function AdminAgenciesPage() {
  ...
  useEffect(() => {
    getAdminAgencies()
      ...
      .catch((err) => {
        setFetchError(err.message || "Error al cargar agencias");
        setLoading(false);
      });
  }, []);
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center text-red-600">
            ...
            <button
              ...
              className="mt-4 px-4 py-2 bg-clay text-white rounded-lg text-sm"
            >
              Reintentar
            </button>
          </div>
      ...
      <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-neutral-500">
              {agencies.length} agencias registradas
            </p>
          </div>
          <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: Grupo Inmobiliario MX"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCreateForm((f) => ({ ...f, legalName: e.target.value }))
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCreateForm((f) => ({
                      ...f,
                      rfc: e.target.value.toUpperCase().slice(0, 13),
                    }))
                  }
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 rounded-lg border text-sm"
                >
                  {PLANS.filter((p) => p !== "inactive").map((p) => (
                ...
                <div className="sm:col-span-2 grid grid-cols-2 gap-4 p-4 bg-clay-50 dark:bg-clay-900/10 rounded-lg">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      min={0}
                      value={createForm.customCredits || 0}
                      onChange={(e) =>
                        setCreateForm((f) => ({
                          ...f,
                          customCredits: Math.max(
                            0,
                            parseInt(e.target.value) || 0,
                          ),
                        }))
        ...
        {agencies.length === 0 && !showCreate ? (
          <div className="text-center py-16 text-neutral-400 space-y-3">
            ...
            <p className="text-sm">
              Haz clic en "+ Nueva agencia" para registrar la primera.
            </p>
          </div>
        ) : (
          <div className="grid gap-4">
            {agencies.map((agency) => (
              <div
                key={agency.id}
                className="p-6 bg-white dark:bg-neutral-900 border rounded-xl space-y-4"
              >
                <div className="flex items-start justify-between flex-wrap gap-4">
                  <div>
                    ...
                    <p className="text-sm text-neutral-500">
                      ...
                      {agency.rfc ? `· RFC: ${agency.rfc}` : ""}
                    </p>
                    <p className="text-sm text-neutral-500">
                      Dueño: {agency.owner?.name} ({agency.owner?.email})
                    </p>
                    <p className="text-sm text-neutral-500">
                      Agentes: {agency._count?.members || 0} /{" "}
                      {agency.agentLimit || 0}
                    </p>
                    {(agency.pricing?.planPrice > 0 ||
                      agency.plan === "custom") && (
                      <p className="text-sm text-clay-600 font-medium">
                        $
                        {(agency.pricing?.planPrice || 0).toLocaleString(
                          "es-MX",
                        )}{" "}
                        MXN / mes
                      </p>
                    ...
                    {agency.subscriptionEnds && (
                      <p
                        className={`text-xs mt-0.5 ${new Date(agency.subscriptionEnds) < new Date() ? "text-red-500 font-medium" : "text-neutral-400"}`}
                      >
                        {new Date(agency.subscriptionEnds) < new Date()
                          ? "⚠️ Vencido: "
                          : "Próximo cobro: "}
                        {new Date(agency.subscriptionEnds).toLocaleDateString(
                          "es-MX",
                        )}
                      </p>
                    )}
                  ...
                  <div className="space-y-2 min-w-48">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="w-20 px-2 py-2 rounded-lg border text-sm bg-white dark:bg-neutral-800"
