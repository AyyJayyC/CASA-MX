...
const ContactRequestModal = dynamic(
  () => import("@/components/ContactRequestModal.jsx"),
);
const MakeOfferModal = dynamic(() => import("@/components/MakeOfferModal.jsx"));
const PropertyAnalytics = dynamic(
  () => import("@/components/analytics/PropertyAnalytics.jsx"),
);
const RentalApplicationForm = dynamic(
  () => import("@/components/RentalApplicationForm.jsx"),
);
const SharePropertyButton = dynamic(
  () => import("@/components/SharePropertyButton.jsx"),
);
const PromotePropertyButton = dynamic(
  () => import("@/components/PromotePropertyButton.jsx"),
);
...
export default function PropertyDetail({ params }) {
  ...
  const financeOptions = Array.isArray(property.financeOptions)
    ? property.financeOptions
    : [];
  ...
  const features = [
    ...
    { icon: "🚿", label: "Baños", value: property.bathrooms ?? "N/D" },
    {
      ...
      value: property.squareMeters
        ? `${Number(property.squareMeters).toLocaleString("es-MX")} m²`
        : "N/D",
    },
    {
      icon: "🌳",
      ...
      value: property.lotSize
        ? `${Number(property.lotSize).toLocaleString("es-MX")} m²`
        : "N/D",
    },
    {
      icon: "🚗",
      ...
      value: property.parkingSpaces
        ? `${property.parkingSpaces} cajones`
        : "N/D",
    },
    {
      icon: "🏢",
      label: "Pisos",
      value: property.floors ? `${property.floors}` : "N/D",
    },
    {
      icon: "📅",
      label: "Año const.",
      value: property.yearBuilt ? `${property.yearBuilt}` : "N/D",
    },
  ];
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-4">
              ...
              {property.status &&
                ...
                property.status !== "disponible" && (
                  <div
                    className={`inline-flex items-center gap-1 px-3 py-1 rounded-md text-sm font-medium ${STATUS_COLORS[property.status] || "bg-slate-100 text-slate-800"}`}
                  >
                    {STATUS_LABELS[property.status]}
            ...
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg p-6 space-y-4">
              ...
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <dt className="text-sm text-neutral-600 dark:text-neutral-400 mb-1">
                    Tipo
                  </dt>
                ...
                <div>
                  ...
                  <dd className="text-xs font-medium text-neutral-900 dark:text-neutral-100 font-mono">
                    {property.id}
                  </dd>
                </div>
                <div>
                  ...
                  {property.seller?.tags &&
                    getOwnerTagsList(property.seller.tags).length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {getOwnerTagsList(property.seller.tags).map((tag) => (
                          <span
                            key={tag}
                            className="px-2 py-0.5 rounded-full text-xs font-medium bg-clay-50 dark:bg-clay-900/20 text-clay-700 dark:text-clay-400"
                          >
                            {getTagLabel(tag)}
                          </span>
                        ))}
              ...
              <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg p-6 space-y-5">
                ...
                <div className="space-y-5">
                  {Object.entries(groupedAmenities).map(
                    ([categoryLabel, categoryAmenities]) => (
