  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((p) => {
      ...
      const matchesQuery =
        ...
        normalizedColonia.includes(q) ||
        normalizedCiudad.includes(q);

      // Area filters
      const matchesEstado =
        !estado || normalizedEstado === String(estado).toLowerCase();
      const matchesCiudad =
        !ciudad || normalizedCiudad === String(ciudad).toLowerCase();
      const matchesColonia =
        !colonia || normalizedColonia.includes(String(colonia).toLowerCase());
      const matchesCodigoPostal =
        !codigoPostal || normalizedCP === String(codigoPostal);
      ...
      const matchesConstructionMeters = (() => {
        ...
        const max = maxConstructionMeters
          ? parseInt(maxConstructionMeters)
          : Infinity;
      ...
      const matchesAmenities =
        ...
        selectedAmenities.every(
          (am) => Array.isArray(p.amenities) && p.amenities.includes(am),
        );
      ...
      const matchesServices =
        ...
        selectedServices.every(
          (svc) =>
            ...
            p.includedServices.includes(svc),
        );
      ...
      const matchesFinancing =
        ...
        selectedFinancing.some(
          (fin) =>
            Array.isArray(p.financeOptions) && p.financeOptions.includes(fin),
        );
