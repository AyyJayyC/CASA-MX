...
export default function PropertyUploadForm({
  ...
  const buildGeocodeQueryFromSuggestion = (typedInput, suggestion) => {
    ...
    const structuredMain = String(
      suggestion?.structured_formatting?.main_text || "",
    ).trim();
    const structuredSecondary = String(
  ...
  const squareMetersRegister = register("squareMeters", {
    valueAsNumber: true,
  });
  const bedroomsRegister = register("bedrooms", { valueAsNumber: true });
  const bathroomsRegister = register("bathrooms", { valueAsNumber: true });
  const securityDepositRegister = register("securityDeposit", {
    valueAsNumber: true,
  });
  const leaseTermMonthsRegister = register("leaseTermMonths", {
    valueAsNumber: true,
  });

  const priceInput = useNumericInput({
    value: watch("price"),
    onValueChange: updateNumericField("price"),
  });
  const monthlyRentInput = useNumericInput({
    value: watch("monthlyRent"),
    onValueChange: updateNumericField("monthlyRent"),
  });
  const squareMetersInput = useNumericInput({
    value: watch("squareMeters"),
    onValueChange: updateNumericField("squareMeters"),
  });
  const bedroomsInput = useNumericInput({
    value: watch("bedrooms"),
    onValueChange: updateNumericField("bedrooms"),
    max: 20,
  });
  const bathroomsInput = useNumericInput({
    value: watch("bathrooms"),
    onValueChange: updateNumericField("bathrooms"),
    max: 20,
  });
  const securityDepositInput = useNumericInput({
    value: watch("securityDeposit"),
    onValueChange: updateNumericField("securityDeposit"),
  });
  const leaseTermMonthsInput = useNumericInput({
    value: watch("leaseTermMonths"),
    onValueChange: updateNumericField("leaseTermMonths"),
    max: 120,
  });
  ...
  const coloniasDisponibles = [
    ...new Set([...coloniasCatalogo, selectedColonia].filter(Boolean)),
  ].sort((a, b) => a.localeCompare(b, "es-MX"));
  ...
  const syncFullAddressFromLocation = useCallback(() => {
    ...
    if (previousParts) {
      const idx = currentAddress
        .toLowerCase()
        .indexOf(previousParts.toLowerCase());
      if (idx >= 0) {
    ...
    const composedAddress = streetPart
      ? `${streetPart}, ${locationParts}`
      : locationParts;
  ...
  const inferLocationFromAddress = useCallback(() => {
    const rawAddress = String(
      getValues("address") || addressSearch || "",
    ).trim();
    ...
    if (!currentCiudad && ciudadesDisponibles.length > 0) {
      ...
      if (detectedCiudad) {
        setValue("ciudad", detectedCiudad, {
          shouldDirty: true,
          shouldValidate: true,
        });
      }
    ...
    if (!currentColonia && coloniasDisponibles.length > 0) {
      ...
      if (detectedColonia) {
        setValue("colonia", detectedColonia, {
          shouldDirty: true,
          shouldValidate: true,
        });
      }
    ...
    if (!currentCp) {
      ...
      if (detectedCp) {
        setValue("codigoPostal", detectedCp, {
          shouldDirty: true,
          shouldValidate: true,
        });
      }
  ...
  const fillFromGeocode = useCallback(
    async (description, typedInput = "", selectedSuggestion = null) => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            getMapsErrorMessage(payload, "No se pudo buscar la direccion."),
          );
        }
        ...
        if (!result) {
          throw new Error(
            "No se obtuvo una ubicacion valida para esta direccion.",
          );
        }
        ...
        if (result.address_components) {
          ...
          const cpFromFormatted =
            String(result.formatted_address || "").match(/\d{5}/)?.[0] || "";
          ...
          let estado =
            ...
            "";
          const cp =
            getComponent(comps, "postal_code") ||
            suggestionComponents.codigoPostal ||
            cpFromFormatted ||
            "";
          ...
          if (!colonia || !ciudad || !estado) {
            const faParts = (result.formatted_address || "")
              .split(",")
              .map((s) => s.trim())
              .filter(Boolean);
            const idxMexico = faParts.findIndex((p) =>
              /mexico|mex|mx/i.test(p),
            );
            const usable =
              idxMexico >= 0 ? faParts.slice(0, idxMexico) : faParts;
            if (usable.length >= 3) {
          ...
          if (
            colonia &&
            estado &&
            colonia.toLowerCase() === estado.toLowerCase()
          ) {
          ...
          const street =
            route && streetNum ? `${route} ${streetNum}` : route || streetNum;
          const typedStreet = getTypedStreetPrefix(
            typedInput || addressSearch,
            description,
          );
          const streetHasNumber = /\d/.test(String(street || ""));
          const effectiveStreet =
            !streetHasNumber && typedStreet ? typedStreet : street;
          const locationParts = [
            colonia,
            ciudad,
            estado,
          ...
          const fullAddress = effectiveStreet
            ...
            : typedStreet
              ? `${typedStreet}, ${locationParts || result.formatted_address || ""}`.replace(
                  /,\s*$/,
                  "",
                )
          ...
          const cpFromDisplay =
            String(result.display_name || "").match(/\b\d{5}\b/)?.[0] || "";
          const estado = nomAddress.state || nomAddress.province || "";
          const ciudad =
            nomAddress.city ||
            nomAddress.town ||
            nomAddress.village ||
            nomAddress.municipality ||
            nomAddress.county ||
            "";
          const colonia =
            nomAddress.neighbourhood ||
            nomAddress.suburb ||
            nomAddress.city_district ||
            "";
          const cp =
            nomAddress.postcode ||
            suggestionComponents.codigoPostal ||
          ...
          const composedAddress = typedStreet
            ? `${typedStreet}, ${locationParts || result.display_name || ""}`.replace(
                /,\s*$/,
                "",
              )
            : result.display_name;
          ...
          if (estado || suggestionComponents.estado)
            setValue("estado", estado || suggestionComponents.estado, {
              shouldDirty: true,
              shouldValidate: true,
            });
          if (ciudad || suggestionComponents.ciudad)
            setValue("ciudad", ciudad || suggestionComponents.ciudad, {
              shouldDirty: true,
              shouldValidate: true,
            });
          if (colonia || suggestionComponents.colonia) {
            setValue("colonia", colonia || suggestionComponents.colonia, {
              shouldDirty: true,
              shouldValidate: true,
            });
          } else {
        ...
        setAddressSearchError(
          e instanceof Error ? e.message : "No se pudo buscar la direccion.",
        );
      }
    },
  ...
  function buildPayload(values, mode = "save") {
    const payload = {
      ...
      ...(mode === "draft"
        ? { status: "incompleto", visibility: "private" }
        : {}),
      ...(listingType === "for_sale"
        ? {
            price: values.price,
  ...
  async function onSubmit(values, mode = "save") {
    try {
      ...
      if (error?.code === "EMAIL_NOT_VERIFIED") {
        alert(
          "Debes verificar tu correo electrónico antes de publicar propiedades. Revisa tu correo y vuelve a intentarlo.",
        );
      } else if (error?.code === "INE_NOT_VERIFIED") {
        alert(
          "Debes subir y verificar tu INE antes de publicar propiedades. Puedes hacerlo en Ajustes de perfil.",
        );
      } else {
        alert(
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {!isEditing &&
                  success.publishEligibility &&
                  !success.publishEligibility.canPublish && (
                    <p className="text-sm text-clay-800 dark:text-clay-300 bg-clay-50 dark:bg-clay-900/20 border border-clay-200 dark:border-clay-800 rounded-lg px-3 py-2">
                      Esta propiedad quedó en borrador. Para publicarla
                      necesitas correo verificado e INE verificada en tu cuenta.
                ...
                {isEditing && (
                  <div className="flex gap-2 mt-3">
                    <Link
                      ...
                      className="px-4 py-2 bg-clay hover:bg-clay-500 text-white text-sm font-semibold rounded-lg transition-colors"
                    >
                      Ver propiedad
                    </Link>
                    {onSave && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <span className="text-xs text-neutral-500 dark:text-neutral-400 block -mt-1 mb-1">
                  Superficie construida / espacio habitable
                </span>
                <input
                  id="squareMeters"
                  {...getNumericInputProps(
                    squareMetersRegister,
                    squareMetersInput,
                  )}
                  className={inputClass}
                  placeholder="0"
              ...
              <div className="relative">
                <input
                  ...
                  aria-activedescendant={
                    activeAddressSuggestionIndex >= 0
                      ? `address-suggestion-${activeAddressSuggestionIndex}`
                      : undefined
                  }
                  onChange={(e) => {
                    ...
                    if (v.length >= 4) {
                      ...
                      if (autocompleteCache.current.has(v)) {
                        ...
                        setActiveAddressSuggestionIndex(
                          cached.length > 0 ? 0 : -1,
                        );
                      ...
                      addressDebounce.current = setTimeout(async () => {
                        try {
                          ...
                          const r = await fetch(
                            `${BACKEND_URL}/maps/autocomplete?${params}`,
                            { credentials: "include" },
                          );
                          ...
                          if (!r.ok) {
                            throw new Error(
                              getMapsErrorMessage(
                                payload,
                                "No se pudo autocompletar la direccion.",
                              ),
                            );
                          }
                ...
                {addressSearchLoading && (
                  <div className="absolute right-3 top-3">
                    <svg
                      className="animate-spin h-4 w-4 text-clay-500"
                      fill="none"
                      viewBox="0 0 24 24"
                    >
                      <circle
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="flex items-start gap-2">
                            <svg
                              className="w-4 h-4 text-clay-500 flex-shrink-0 mt-0.5"
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
                              <path
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2 mt-1">
                                {Object.entries(PARKING_TYPE_LABELS).map(
                                  ([value, label]) => (
                                    <label
                                      key={value}
                                      className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border cursor-pointer text-sm transition-colors ${
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2 mt-1">
                                {Object.entries(FURNISHED_LABELS).map(
                                  ([value, label]) => (
                                    <label
                                      key={value}
                                      className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border cursor-pointer text-sm transition-colors ${
                            ...
                            <div>
                              ...
                              <p className="text-xs text-neutral-500 mt-1">
                                {watch("status") === "preventa" ||
                                watch("status") === "en_remodelacion"
                                  ...
                                  : watch("status") === "bajo_promesa"
                                    ...
                                    : watch("status") === "vendido" ||
                                        watch("status") === "rentado" ||
                                        watch("status") === "retirado"
              ...
              {shouldShowAddressEmptyState && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-lg z-20 p-4">
                  <p className="text-sm font-medium text-neutral-800 dark:text-neutral-100">
                    No encontramos sugerencias claras en Mexico para esa
                    direccion.
                  </p>
                  <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
                    Agrega ciudad o estado para mejorar el resultado, por
                    ejemplo: Hermosillo, Sonora.
                  </p>
                </div>
              )}
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      id="leaseTermMonths"
                      {...getNumericInputProps(
                        leaseTermMonthsRegister,
                        leaseTermMonthsInput,
                      )}
                      className={inputClass}
                      placeholder="12"
                    />
                  ...
                  <div className="sm:col-span-2 space-y-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50/70 dark:bg-neutral-900/50 p-4">
                    ...
                    <RentalServicesSelector
                      ...
                      onChange={(nextValue) =>
                        setValue("includedServices", nextValue, {
                          shouldDirty: true,
                          shouldValidate: true,
                        })
                      }
                  ...
                  <div className="sm:col-span-2 space-y-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50/70 dark:bg-neutral-900/50 p-4">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
                        Incluye equipo y comodidades comunes en rentas
                        amuebladas o largas estancias.
                      </p>
                    </div>
                    <PropertyAmenitiesSelector
                      selectedAmenities={selectedAmenities}
                      onChange={(nextValue) =>
                        setValue("amenities", nextValue, {
                          shouldDirty: true,
                          shouldValidate: true,
                        })
                      }
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                    {photoFiles.map((photo, index) => (
                      <div
                        key={`${index}-${String(photo).slice(0, 24)}`}
            ...
            <div className="pt-4 border-t border-neutral-200 dark:border-neutral-800">
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                <span className="text-sm text-neutral-700 dark:text-neutral-300 leading-snug">
                  Certifico que soy el propietario o tengo autorización legal
                  para publicar esta propiedad, y que la información
                  proporcionada es verídica. Acepto que deberé subir documentos
                  que acrediten la propiedad y que la publicación permanecerá
                  pendiente de verificación hasta su revisión.
                </span>
              </label>
              {errors.confirmedPermissions && (
                <p className="text-red-500 text-sm mt-2">
                  {errors.confirmedPermissions.message}
                </p>
              )}
            </div>
